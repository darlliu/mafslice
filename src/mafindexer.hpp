...
public:
  ...
  inode(const unsigned &ll, const unsigned &rr, short pp = 0)
      : l(ll), r(rr), p(pp){};
...
typedef member_hook<inode, avl_set_member_hook<>, &inode::member_hook_>
    MemberOption;
...
class mafdb : public seqdb {
  friend class boost::serialization::access; // enable boost serialize and be a
                                             // lazy programmer
public:
  ...
  std::string get(const std::string &chr, const unsigned &l,
                  const unsigned &r) {
    set_chr(chr);
    return get(l, r);
  };
